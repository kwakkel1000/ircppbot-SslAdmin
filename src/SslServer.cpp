...
SslServer::SslServer(boost::asio::io_service& io_service, unsigned short port) : io_service_(io_service), acceptor_(io_service, boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), port)), context_(io_service, boost::asio::ssl::context::sslv23)
{
	...
	acceptor_.async_accept(new_session->socket(), boost::bind(&SslServer::handle_accept, this, new_session, boost::asio::placeholders::error));
}
...
void SslServer::handle_accept(Session* new_session, const boost::system::error_code& error)
{
	if (!error)
	{
		...
		boost::bind(&SslServer::handle_accept, this, new_session,
